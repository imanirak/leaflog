          ...
          <div className="flex items-end justify-between">
            <div className="flex items-end gap-5">
              ...
              <div className="relative">
                ...
                {profile?.is_public && (
                  <div className="absolute -bottom-2 -right-2 rounded-full px-1.5 py-0.5 text-xs font-bold text-white" style={{ background: "#16a34a" }}>
                    ✓
                  </div>
                )}
              </div>
      ...
      <div className="px-8 py-6">
        ...
        {!profile && (
          <div className="mb-6 flex items-center gap-4 rounded-2xl p-5 shadow-sm" style={{ background: "var(--card)", border: "1px solid #fed7aa" }}>
            <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl text-xl" style={{ background: "var(--orange-light)" }}>🪪</div>
        ...
        <div className="mb-4 flex items-center justify-between">
          ...
          {photos.length > 0 && (
            <span className="rounded-full px-3 py-1 text-xs font-semibold" style={{ background: "var(--orange-light)", color: "var(--orange)" }}>
        ...
        {photos.length === 0 ? (
          <div className="flex flex-col items-center justify-center rounded-2xl py-20 text-center" style={{ background: "var(--card)", border: "2px dashed var(--border)" }}>
            <div className="mb-3 flex h-16 w-16 items-center justify-center rounded-2xl text-3xl" style={{ background: "var(--cream)" }}>📸</div>
          ...
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-3">
            {photos.map(photo => (
              <div key={photo.id} className="group overflow-hidden rounded-2xl shadow-sm transition-all hover:shadow-md hover:-translate-y-0.5" style={{ background: "var(--card)" }}>
                ...
                <div className="p-3.5">
                  {photo.plants && (
                    <Link href={`/app/plants/${photo.plants.id}`} className="text-sm font-semibold hover:text-orange-500 transition-colors" style={{ color: "var(--text)" }}>
                      {photo.plants.name}
                    </Link>
                  )}
                  ...
                  <div className="mt-2 flex items-center justify-between">
                    ...
                    <form action={unsharePhoto.bind(null, photo.id, photo.plants?.id ?? "")}>
                      <button type="submit" className="text-xs transition-colors hover:text-red-500" style={{ color: "#d1d5db" }}>
                        Unshare
                      </button>
                    </form>
                  </div>
