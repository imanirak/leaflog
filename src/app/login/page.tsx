  ...
  return (
    <AuthShell floral>
      <div className="mb-7 flex flex-col items-center text-center">
        <div
          className="mb-4 flex h-14 w-14 items-center justify-center rounded-2xl text-2xl shadow-md"
          style={{ background: `linear-gradient(135deg, ${FOREST}, ${SAGE})` }}
          aria-hidden="true"
        >
          🌿
        </div>
        <h1 className="text-2xl font-bold" style={{ color: FOREST_DARK, fontFamily: "var(--font-display)" }}>Welcome back</h1>
        <p className="mt-1.5 max-w-[15rem] text-sm leading-relaxed" style={{ color: MUTED_GREEN }}>
          Sign in to pick up your plant diary right where you left off.
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-3.5" noValidate>
        ...
        <div className="flex justify-end">
          <Link href="/login" className="text-xs font-medium hover:underline" style={{ color: SAGE }}>
            Forgot password?
          </Link>
        </div>
      ...
      <p className="mt-6 text-center text-sm" style={{ color: MUTED_GREEN }}>
        No account?{" "}
        <Link href="/signup" className="font-medium hover:underline" style={{ color: SAGE }}>
          Sign up
        </Link>
      </p>
    </AuthShell>
